...
class Blip2VicunaInstructPEFTModel(Blip2Base):
    """
    ...
    def __init__(
        ...
        # unfreeze_layers = ['bert.encoder.layer.10','bert.encoder.layer.11','cls.predictions']
        unfreeze_layers = []
        ...
        peft_config = LoraConfig(
            ...
            r=self.cfg.get("lora_r"),
            lora_alpha=self.cfg.get("lora_alpha"),
            lora_dropout=self.cfg.get("lora_dropout"),
        ...
        self.llm_model.base_model.model.linear_layer.float()
        self.llm_model.base_model.model.linear_layer.weight.requires_grad=True
        self.llm_model.base_model.model.linear_layer.bias.requires_grad=True
    ...
    def forward(self, samples):
        ...
        image = samples["image"] #torch.Size([8, 3, 224, 224])
        ...
        # print(self.llm_tokenizer.batch_decode(
        #     [[ 3869, 29889,     2],
        #     [ 1939, 29889,     2],
        #     [ 1939, 29889,     2],
        #     [ 1939, 29889,     2],
        ...
        with self.maybe_autocast():
            outputs = self.llm_model(
                ...
                return_dict=True,
                labels=targets,
                output_hidden_states=self.cfg.get("output_hidden_states"), #True gzh add here
                contrastive_layer=self.cfg.get("contrastive_layer"),  # gzh add here
                sim_type=self.cfg.get("sim_type"), # gzh add here
                use_cluster=self.cfg.get("use_cluster"),
                use_online_ranking=self.cfg.get("use_online_ranking"),
                cluster_margin=self.cfg.get("cluster_margin"), # gzh add here
                cluster_metric=self.cfg.get("cluster_metric"), # gzh add here
                cluster_scale=self.cfg.get("cluster_scale"), # gzh add here
                use_triplet=self.cfg.get("use_triplet"),
                triplet_margin=self.cfg.get("triplet_margin"), # gzh add here
                triplet_metric=self.cfg.get("triplet_metric"), # gzh add here
                triplet_scale=self.cfg.get("triplet_scale"), # gzh add here
            )
        ...
        return {"loss_o": loss_o, "loss_c": loss_c, "loss_t": loss_t}
    
    def forward_tsne(self, samples):
        # print('-----------------')
        # print(samples["text_input"])
        # print(samples["text_output"])
        # print('-----------------')
        ...
        with self.maybe_autocast():
            outputs = self.llm_model(
                ...
                return_dict=True,
                labels=targets,
                output_hidden_states=True, #True gzh add here
                plt_tsne = True,
                contrastive_layer=self.cfg.get("contrastive_layer"),  # gzh add here
                sim_type=self.cfg.get("sim_type"), # gzh add here
                use_cluster=self.cfg.get("use_cluster"),
    ...
    def generate(
        ...
        num_captions=1,
        temperature=1,
        roc_auc_compute=False, # True, mannually added by gzh for roc_auc compute, output vanilla output logits
        output_scores=False, # True, added for scores return
        return_dict_in_generate=False, # True, added for scores return
    ):
        ...
        with self.maybe_autocast():
            ...
            outputs = self.llm_model.generate(
                ...
                # roc_auc_compute=roc_auc_compute, # True, mannually added by gzh for roc_auc compute, output vanilla output logits
                output_scores=output_scores, # True, added for scores return
                return_dict_in_generate=return_dict_in_generate, # True, added for scores return
                num_beams=num_beams,
        ...
        if roc_auc_compute:
            ...
            label_class = torch.tensor([1939,3869]).cuda() # no, yes
            scores = torch.index_select(scores, 1, label_class)
            scores = nn.functional.softmax(scores, dim=-1)

            # Perform a word replacement 
            seq = outputs.sequences
            for i in range(scores.size(0)):
                if label_class[torch.argmax(scores[i]).item()] != seq[i][1]:
                    seq[i][1] = label_class[torch.argmax(scores[i]).item()]

            seq[seq == 0] = 2 # convert output id 0 to 2 (eos_token_id)
            output_text = self.llm_tokenizer.batch_decode(
                seq, skip_special_tokens=True
            )

            return (output_text, scores[:, 1])
