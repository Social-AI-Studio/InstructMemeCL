...
@registry.register_processor("blip_meme")
class BlipMemeProcessor(BaseProcessor):
    def __init__(self, max_words=50):
        self.max_words = max_words

    def __call__(self, question):
        return self.pre_meme(question)
    ...
    def pre_meme(self, question):
        # 去除Email
        question = re.sub(r'^([\w]+\.*)([\w]+)\@[\w]+\.\w{3}(\.\w{2}|)', ' ', question)
        # 去除url
        question = re.sub(r'^(https:\S+)', ' ', question)
        question = re.sub(r'[a-zA-Z]+://[^\s]*', ' ', question)

        #通过infer反推train，text_processor应该在qformer tokenizer和llm tokenizer处理的，只有image需要处理
